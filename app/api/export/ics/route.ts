...
export async function GET(req: NextRequest) {
  ...
  const ve = (uid: string, dtStart: string, dtEnd?: string, summary = "Event") =>
    ["BEGIN:VEVENT",`UID:${uid}`,`DTSTART:${dtStart.replace(/[-:]/g,"").split(".")[0]}Z`,
      dtEnd ? `DTEND:${dtEnd.replace(/[-:]/g,"").split(".")[0]}Z` : undefined,
      `SUMMARY:${summary}`,"END:VEVENT"].filter(Boolean) as string[];
  tasks.forEach(t => lines.push(...ve(`task-${t.id}@ju`, (t.startAt ?? t.dueAt ?? new Date()).toISOString(), t.dueAt?.toISOString(), `Task: ${t.title}`)));
  shifts.forEach(s => lines.push(...ve(`shift-${s.id}@ju`, s.startAt.toISOString(), s.endAt.toISOString(), `Schicht: ${s.title}`)));
